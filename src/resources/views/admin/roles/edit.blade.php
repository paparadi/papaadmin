...
<div class="col-md-8 col-sm-12 grid-margin stretch-card">
	<div class="card">
		<div class="card-body">
			<h4 class="card-title"><b>Edit Role</b></h4>
			<form class="forms-sample" method="POST" action="{{route('admin.roles.update', ['role' => $role->id])}}">
				@csrf
				@method('PUT')
			  	<div class="form-group">
					<label for="name">Name</label>
					<input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" value="{{old('name') ?? $role->name}}" placeholder="Name">
					@error('name') 
						...
						@foreach ($permissions as $one)
							...
							<option 
								...
								@if(!empty(old('permissions')))
									@if(in_array($one->name, old('permissions'))) 
										{{'selected'}} 
									@endif
								@else 
									@if($role->hasPermissionTo($one->name)) 	
										{{'selected'}} 
									@endif
								@endif
								>
